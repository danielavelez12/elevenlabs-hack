    ...
    async def transcribe_audio_stream(
        ...
        try:
            await asyncio.get_event_loop().run_in_executor(
                None, lambda: self._run_with_cleanup(audio_processor, config, settings)
            )
        except Exception as e:
            print(f"Error during transcription: {str(e)}")
            raise

    def _run_with_cleanup(self, audio_processor, config, settings):
        try:
            self.ws_client.run_synchronously(audio_processor, config, settings)
        finally:
